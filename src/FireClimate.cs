...
namespace Landis.Library.Climate
{
    public static class FireClimate
    {
        ...
        {
            // Check to make sure all fire parameters were included in input file
            // SpringStart and WinterStart are optional. Julian days will be used if not included in input file
            // RHSlopeAdjust is the only required variable for calculating FireWeatherIndex

            // Initilize all fire variables from input paramters
            ...
            {
                throw new FireWeatherCalculationException("TemportalGranularity not set");
            }
                ...
                if (ecoregion.Active)
                {
                    ...
                    for (int timestep = 0; timestep < maxtimestep; timestep++)
                    {
                        if (timestep >= SpringStart && timestep < WinterStart)
                        {
                            ...
                            //Climate.Future_DailyData[year][ecoregion.Index].DailyFireWeatherIndex[timestep] = FireWeatherIndex;
                        ...
                        else
                        {
                            ...
                            //Climate.Future_DailyData[year][ecoregion.Index].DailyFireWeatherIndex[timestep] = 0;
                        }
                    }
                }
            }
